...
public abstract class JpaUtils {
	...
	private static Pattern ALIAS_PATTERN = Pattern.compile(
			ALIAS_PATTERN_STRING, Pattern.CASE_INSENSITIVE);
	private static String FROM_PATTERN_STRING = "(from.*+)";
	private static Pattern FROM_PATTERN = Pattern.compile(FROM_PATTERN_STRING,
	 ...
	 */
	public static synchronized <T> String getOrCreateAlias(
	...
	public static <T, K> Join<T, K> findJoinedType(CriteriaQuery<T> query,
			Class<T> rootClass, Class<K> joinClass) {
	...
	public static Path<?> getPath(Path<?> path, String propertyPath) {
		...
		String name = StringUtils.substringBefore(propertyPath,
				PropertyUtils.PROPERTY_SEPARATOR);
	...
	public static String createCountQueryString(String queryString) {
		return queryString
				.replaceFirst("^.*(?i)from", "select count (*) from ");
	}
	 ...
	 */
	public static String addOrder(String queryString, String propertyPath,
			boolean asc) {
